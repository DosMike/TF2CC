  ...
  var Alerty = function() {
    ...
    var Dialog = {
      ...
      setup: function(type, content, opts, onOk, onCancel) {
        ...
        this.bindEvent($modal, $overlay, onOk, onCancel); // see next
      },
       ...
       */
      bindEvent: function($modal, $overlay, onOk, onCancel) {
        var that = this;
        var $btnOk = $modal.querySelector('.btn-ok');
        var $btnCancel = $modal.querySelector('.btn-cancel');
        ...
        if (!commonUse.hasClass($modal, 'toasts')) {
          commonUse.addEvent($overlay, 'click', function(event) {
            if (event.target == $overlay && commonUse.hasClass($modal, 'alerty-show')) that.close($modal, onCancel);
          });
        }
      },
